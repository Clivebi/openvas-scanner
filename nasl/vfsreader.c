...
#define mutext_init(a, b)
#define mutext_lock(a)
#define mutext_unlock(a)
#define mutext_destory(a)
#endif
...
int
vfs_file_size (vfs_dir_entry_p entry)
{
  return entry->file_size;
}

char *
vfs_file_name (vfs_dir_entry_p entry)
{
  return entry->name;
}
...
int
init_vfs_dir_entry_array (vfs_dir_entry_array_p array,
                          unsigned int allocate_size)
{
  array->size = 0;
  array->allocate_size = allocate_size;
  array->data = (vfs_dir_entry_p *) VFS_MALLOC (sizeof (vfs_dir_entry_p)
                                                * array->allocate_size);
  if (array->data == NULL)
    {
...
void
free_vfs_dir_entry_array (vfs_dir_entry_array_p array)
{
  for (unsigned int i = 0; i < array->size; i++)
    {
      VFS_FREE (array->data[i]);
    }
  VFS_FREE (array->data);
}

int
extend_vfs_dir_entry_array (vfs_dir_entry_array_p array, unsigned int add_size)
{
  ...
  vfs_dir_entry_p *new_data =
    (vfs_dir_entry_p *) VFS_MALLOC (new_size * sizeof (vfs_dir_entry_p));
  if (new_data == NULL)
    {
      return ERROR_NO_MEMORY;
    }
  ...
  memcpy (new_data, array->data,
          array->allocate_size * sizeof (vfs_dir_entry_p));
...
int
add_vfs_dir_entry (vfs_dir_entry_array_p array, vfs_dir_entry_p entry)
{
...
int
move_dir_entry (vfs_dir_entry_array_p to_array,
                vfs_dir_entry_array_p from_array)
{
  ...
  memcpy (&to_array->data[to_array->size], from_array->data,
          from_array->size * sizeof (vfs_dir_entry_p));
...
int
vfs_unpack_files (FILE *file, vfs_dir_entry_array_p dirs,
                  vfs_dir_entry_array_p files)
{
  size_t read_size = 0;
  ...
  do
    {
      ...
      entry =
        (vfs_dir_entry_p) VFS_MALLOC (sizeof (vfs_dir_entry_t) + name_size);
      if (entry == NULL)
        {
...
int
vfs_unpack (FILE *file, vfs_handle_p vfs)
{
  int result = 0;
...
// open a vfs object
vfs_handle_p
vfs_open (const char *host_file_path, void *dic, int dic_size)
#else
vfs_handle_p
vfs_open (const char *host_file_path)
#endif
{
  FILE *host_file = NULL;
  ...
  host_file = fopen (host_file_path, "r");
  if (host_file == NULL)
    {
      return NULL;
    }
  do
    {
      vfs = (vfs_handle_p) VFS_MALLOC (sizeof (vfs_handle_t));
      if (vfs == NULL)
        {
          break;
        }
      ...
      for (unsigned int i = 0; i < vfs->files.size; i++)
        {
...
vfs_dir_entry_p *
vfs_get_all_files (vfs_handle_p vfs, int *count)
{
...
int
vfs_get_file_full_path (vfs_handle_p vfs, vfs_dir_entry_p entry,
...
int
vfs_lookup_name_helper (vfs_dir_entry_array_p array, const char *name)
{
...
vfs_lookup (vfs_handle_p vfs, const char *full_path)
{
  ...
  for (unsigned int i = index; i < vfs->files.size; i++)
    {
      entry = vfs->files.data[i];
      if (strcmp (entry->name, base))
        {
...
int
vfs_raw_read_at (vfs_handle_p vfs, void *buf, long offset, int size)
{
  int error = -1;
  if (size == 0)
    {
...
#ifdef __USE_ZLIB__
void *
vfs_get_file_all_content (vfs_handle_p vfs, vfs_dir_entry_p entry, int *error_p)
{
  ...
  do
    {
      raw = VFS_MALLOC (entry->compress_size);
      uncompress_buf = VFS_MALLOC (entry->file_size + 4);
      ...
      if (entry->compress_size
          != (unsigned int) vfs_raw_read_at (vfs, raw, entry->offset,
                                             entry->compress_size))
        {
  ...
  if (error == ERROR_SUCCESS)
    {
      ((char *) uncompress_buf)[entry->file_size] = 0;
      ((char *) uncompress_buf)[entry->file_size + 1] = 0;
      ((char *) uncompress_buf)[entry->file_size + 2] = 0;
      ((char *) uncompress_buf)[entry->file_size + 3] = 0;
      return uncompress_buf;
    }
  if (uncompress_buf != NULL)
    {
      VFS_FREE (uncompress_buf);
    }
  return NULL;
}
...
#else
void *
vfs_get_file_content (vfs_handle_p vfs, vfs_dir_entry_p entry, int offset,
                      int size, int *read_size, int *error)
{
  void *content = NULL;
...
void *
vfs_get_file_all_content (vfs_handle_p vfs, vfs_dir_entry_p entry, int *error)
{
...
void *
vfs_read_file_content (const char *path, int *dic_size)
{
  FILE *f = NULL;
  void *content = NULL;
